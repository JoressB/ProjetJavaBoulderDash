...
import contract.LevelLoad;
import contract.ModelConvert;
...
import model.GameInformationModel;
 ...
 * the view when there are changes on it. The different XML level file are also saved in the project database
 *
...
public class Level extends Observable implements Runnable {
	 ...
	 * @param  audioLoad  Audio load helper
     * @param  mode             Instance mode
	 */
	public Level(String levelName, AudioLoad audioLoad, String mode) {
		...
		this.levelLoad = new LevelLoad(this.levelName);

		this.groundGrid = this.levelLoad.getGroundGrid();
		this.sizeWidth = this.levelLoad.getWidthSizeValue();
		this.sizeHeight = this.levelLoad.getHeightSizeValue();

		this.pointer = new Pointer();
		this.gameInformationModel = new GameInformationModel(this.levelLoad.getDiamondsToCatch());
     ...
     * @param  audioLoad  Audio load helper
     */
    public Level(String levelName, AudioLoad audioLoad) {
        this(levelName, audioLoad, "game");
    }
	 ...
	 * @param audioLoad  Audio load helper
	 */
	public Level(AudioLoad audioLoad) {
		this.audioLoad = audioLoad;
		...
		for (int x = 0; x < this.sizeWidth; x++) {
			for (int y = 0; y < this.sizeHeight; y++) {
				this.groundGrid[x][y] = new Ground();
			}
		}
	...
	private void initRockford() {
		this.rockfordPositionX = this.levelLoad.getRockfordPositionX();
		this.rockfordPositionY = this.levelLoad.getRockfordPositionY();
		this.rockford = this.levelLoad.getRockfordInstance();
	}
	...
	private void createLimits() {
		...
		for (int x = 0; x < this.sizeWidth; x++) {
			this.groundGrid[x][0] = new Wall();
			this.groundGrid[x][maxHeight] = new Wall();
		}
		for (int y = 0; y < this.sizeHeight; y++) {
			this.groundGrid[0][y] = new Wall();
			this.groundGrid[maxWidth][y] = new Wall();
		}
	}
	...
	private void playCollisionSound(int posX, int posY) {
		...
		if (collisionSound != null) {
			this.audioLoad.playSound(collisionSound);
		}
	}
	...
	public void setPositionOfRockford(int posX, int posY) {
		...
		if (this.isOutOfBounds(posX, posY) == false) {
			...
			// Save the x / y pos of Rockford in the level only
	...
	public void triggerBlockChange(String blockValue) {
		...
		// Grab model value
		ModelConvert modelConverter = new ModelConvert();
	 ...
	 */
	public LevelLoad getLevelLoadHelper() {
	...
	/**
	 * Makes the Rock[x][y] slide right
	 *
	 * @param  x  Object horizontal position
	 * @param  y  Object vertical position
	 */
	...
	/**
	 * Makes the Rock[x][y] transform into a diamond
	 *
	 * @param  x  Object horizontal position
	 * @param  y  Object vertical position
	 */
	public void transformThisBoulderIntoADiamond(int x, int y) {
		this.groundGrid[x][y + 2] = new Diamond();
		this.groundGrid[x][y] = new EmptyModel();
	}

	/**
	 * Makes the Rock[x][y] moving to right
	 *
	 * @param  x  Object horizontal position
	 * @param  y  Object vertical position
	 */
	...
	/**
	 * Makes the Rock[x][y] moving to left
	 *
	 * @param  x  Object horizontal position
	 * @param  y  Object vertical position
	 */
	...
	/**
	 * Expand the LeftWall to left
	 *
	 * @param  x
	 * @param  y
	 */
	public void expandThisWallToLeft(int x, int y) {
		this.groundGrid[x - 1][y] = new LeftWall();
	}

	/**
	 * Expand the LeftWall to right
	 * 
	 * @param  x
	 * @param  y
	 */
	...
	/**
	 * Get the mode where this level is used
	 * 
	 * @return mode
	 */
	...
	/**
	 * Set the mode where this level is used
	 * 
	 * @param mode
	 */
